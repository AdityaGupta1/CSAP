...
*/
public class MultipleTurtleRun_4Gupta extends GraphicsProgram {
   GTurtle[] turtles;
   ...
   public void run() {
      ...
      while (!isValid) {
         input = JOptionPane.showInputDialog("How many turtles (2 to 9)?");
      
         try {
            numberOfTurtles = Integer.parseInt(input);
         } catch (NumberFormatException e) {
            continue;
         }
         
         if (numberOfTurtles < 2 || numberOfTurtles > 9) {
            continue;
      ...
      turtles = new GTurtle[numberOfTurtles];
      
      for (int i = 0; i < turtles.length; i++) {
         turtles[i] = new GTurtle(100, 50 + (i * (400 / turtles.length)));
      }
   ...
   public static void main(String[] args) {
      new MultipleTurtleRun_4Gupta().start(args);
   }
}